...
import { Parallax, Reveal, Scrub3D, Tilt, useHeroParallax, HorizontalScroll } from './parallax';
...
export function HomePage({ recipes, articles }: { recipes: Recipe[]; articles: Article[] }) {
  ...
  return (
    <Shell page="home" active="/">
      {/* Hero Section — Layered entrance + parallax video */}
      <section className="pt-0 pb-3 md:pb-10">
        <div className="relative flex min-h-[520px] items-end overflow-hidden px-[20px] pb-[28px] pt-[160px] isolate before:absolute before:inset-0 before:-z-10 before:bg-gradient-to-b before:from-[#f2f0df]/85 before:via-[#f5f2e6]/92 before:to-[#f5f2e6]/96 md:aspect-[1.83] md:min-h-0 md:items-center md:px-[96px] md:py-[72px] md:before:bg-gradient-to-r md:before:from-[#f2f0df]/96 md:before:via-[#f5f2e6]/68 md:before:to-[#f5f2e6]/0">
          <div
            ref={bgRef}
            className="absolute inset-0 -z-20 will-change-transform parallax-image"
      ...
      {/* Recipe Cards — Staggered horizontal 3D reveal (left, center, right) */}
      <Reveal variant="up" y={32} delay={100}>
        <ListingSection
        ...
        <div className="mx-auto max-w-[1180px] px-[18px] md:px-[26px]">
          <Reveal variant="up" y={32} className="mealprep-split">
            <div className="grid overflow-hidden rounded-2xl bg-white shadow-[0_8px_32px_rgba(47,52,45,0.10)] md:grid-cols-[1fr_1.1fr]">
              ...
              <div className="mealprep-image relative order-first h-[260px] overflow-hidden md:order-none md:h-auto md:min-h-[420px]">
                <Parallax speed={0.18} className="absolute inset-0">
                  <img
                    className="h-[130%] w-full object-cover parallax-image"
      ...
      {/* Articles Section — Horizontal scroll gallery on desktop */}
      <Reveal variant="up" y={32} delay={100}>
        <ListingSection
          ...
          href="/articles"
        >
          <HorizontalScroll speed={0.6} className="md:grid md:grid-cols-3 md:gap-[22px] md:horizontal-scroll-section-hidden">
            {latest.slice(0, 6).map((a, i) => (
              <Reveal
                ...
                delay={i * 80}
                y={24}
                className={`reveal-stagger-${i + 1} md:flex md:flex-col md:w-[calc(33.333%-16px)] md:min-w-[280px]`}
              >
                <Tilt max={9} scale={1.03}>
      ...
      <Reveal variant="zoom" y={20} delay={100} className="newsletter-reveal">
        <NewsletterSection />
      </Reveal>
    </Shell>
  );
